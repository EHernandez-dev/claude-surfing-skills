...
{Dashboard note: this template is the canonical research report written by the Report Writer agent. The per-spot **Dashboard** (`render_report.py --mode dashboard`, Step 6C) is a separate artifact: a tabbed HTML page (Today / Forecast / Windows / Spot info) plus a paired flat `.md` twin, both generated deterministically by the renderer from the Step 5A data package, not from this template. The twin stacks the four views (Markdown has no tabs); its **Today** section mirrors the same target-day fields used by the hero and tide chart below (`analysis.target_day.verdict`, `analysis.target_day.one_liner`, `analysis.target_day.windows`, and the target day's `conditions.tides` events), and its **Forecast** section lists `analysis.week` (one swell / wind / verdict row per day; the HTML tab is interactive, with a Week-at-a-glance tide overview plus a per-day drilldown, but the flat twin just lists the seven days), so the renderer's output stays in sync with the placeholders here per the CLAUDE.md data-contract rule. Windows / Spot info are placeholders the renderer fills in later slices.}
