...
{Dashboard note: this template is the canonical research report written by the Report Writer agent. The per-spot **Dashboard** (`render_report.py --mode dashboard`, Step 6C) is a separate artifact: a tabbed HTML page (Today / Forecast / Windows / Spot info) plus a paired flat `.md` twin, both generated deterministically by the renderer from the Step 5A data package, not from this template. The twin stacks the four views (Markdown has no tabs); its **Today** section mirrors the same target-day fields used by the hero and tide chart below (`analysis.target_day.verdict`, `analysis.target_day.one_liner`, `analysis.target_day.windows`, and the target day's `conditions.tides` events), its **Forecast** section lists `analysis.week` (one swell / wind / verdict row per day; the HTML tab is interactive, with a Week-at-a-glance tide overview plus a per-day drilldown, but the flat twin just lists the seven days), and its **Windows** section lists `analysis.windows` (the week's session windows in date order, each with its recommended time, verdict, swell / wind and reasoning; the HTML tab additionally draws each day's tide chart with the session shaded, but the flat twin just lists them), and its **Spot info** section carries the spot's standing context, flattened from the HTML tab's facts strip + dossier (the facts as lines - break, level, water temp, `spot_data.water_quality` status, the target day's `conditions.weather` UV index, `spot_data.lifeguards`, profile age - then the works-on profile from `spot_data.profile` with the profile-freshness note and a re-research prompt when stale or undated from `conditions.spot.profile` plus any applied `conditions.bias`, The Wave with `spot_data.peaks`, `spot_data.hazards`, the assigned `conditions.buoy` and `conditions.sea_temperature` with the dated water-quality summary, `spot_data.access` / `spot_data.rentals` / `spot_data.food` with Google Maps links as Markdown links, `spot_data.nearby_spots`, `spot_data.community_notes` with an explicit "no recent first-hand reports found" empty state, a Location Google Maps link + coordinates, and `spot_data.webcams`), so the renderer's output stays in sync with the placeholders here per the CLAUDE.md data-contract rule.}
...
- **Parking:** {situation, fees; link the lot on Google Maps when its coordinates were found}
- **Transit:** {how to arrive without a car (train/metro/bus line and stop, walk time), or "no transit option noted"}
